...
async function main() {
  ...
  // 1. Clean up DW tables in dependency-safe order (Facts and dependent tables first)
  ...
  // Delete from tables that reference dimensions before deleting dimensions themselves
  try {
    // This table references dim_category, so it must be cleared first.
    await prisma.reports_metadata.deleteMany({});
  } catch (e) {
    // Silently ignore if the table doesn't exist.
  }

  // Now, it's safe to delete the dimension tables.
  ...
  for (let i = 1; i <= NUM_FACTS_PER_TABLE; i++) {
    ...
    if (
      ...
      randomWasteType.waste_type_name &&
