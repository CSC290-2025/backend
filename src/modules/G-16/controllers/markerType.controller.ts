...
export const getAllMarkerTypes = async (c: Context) => {
  ...
  return successResponse(
    ...
    200,
    'Marker types retrieved successfully'
  );
};
...
export const getMarkerTypesByType = async (c: Context) => {
  ...
  if (
    markerTypeIdNumber < 1 ||
    markerTypeIdNumber > 8 ||
    !Number.isInteger(markerTypeIdNumber)
  ) {
    throw new ValidationError(
      'markerType_id must be an integer between 1 and 8.'
    );
  }

  const marker = await markerTypeService.getMarkerTypesByTypeService([
    markerTypeIdNumber,
  ]);
  return successResponse(
    c,
...
export const getMarkerTypeByTypes = async (c: Context) => {
  ...
  if (!Array.isArray(marker_type_ids) || marker_type_ids.length === 0) {
    throw new ValidationError(
      'marker_type_ids must be an array and contain at least one item.'
    );
  }
  ...
  for (const id of marker_type_ids) {
    if (typeof id !== 'number' || !Number.isInteger(id) || id < 1 || id > 8) {
      throw new ValidationError(
        'All marker_type_ids must be integers between 1 and 8.'
      );
    }
  }
  const marker =
    await markerTypeService.getMarkerTypesByTypeService(marker_type_ids);
  return successResponse(
    c,
    { marker: marker, count: marker.length },
    200,
    'Marker types retrieved successfully'
  );
};
...
export const getMarkerTypesInBounds = async (c: Context) => {
  ...
  if (minLat < -90 || maxLat > 90 || minLng < -180 || maxLng > 180) {
    throw new ValidationError(
      'Bounds are outside the valid range: Latitude (-90 to 90), Longitude (-180 to 180).'
    );
  }

  if (minLat >= maxLat || minLng >= maxLng) {
    throw new ValidationError(
      'The min value must be less than the max value for both latitude and longitude.'
    );
  }
  ...
  return successResponse(
    ...
    { marker: marker, count: marker.length },
    200,
    'Markers within bounds retrieved successfully'
  );
};
...
export const updateMarkerType = async (c: Context) => {
  ...
  if (
    isNaN(markerIdNumber) ||
    markerIdNumber < 1 ||
    !Number.isInteger(markerIdNumber)
  ) {
    throw new ValidationError(
      'Invalid Marker ID (must be a positive integer).'
    );
  }

  const validatedData = await c.req.json();
  const marker = await markerTypeService.updateMarkerTypeService(
    markerIdNumber,
    validatedData
  );
  ...
  if (!marker) {
    return successResponse(
      ...
      null,
      404,
      `Marker with ID ${markerIdNumber} not found.`
    );
  }
  ...
  return successResponse(
    ...
    { marker },
    200,
    'Marker Type updated successfully.'
  );
};
...
export const deleteMarkerType = async (c: Context) => {
  ...
  const deletionResult =
    await markerTypeService.deleteMarkerType(markerIdNumber);

  return successResponse(
    c,
    { deletionResult },
    200,
    'Marker Deleted Succesfully'
  );
};
